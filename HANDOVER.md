...
| `route-ab-sync-coroutine` | **v1.2.2-ab** | ✅ 已实测 | A+B 路线（保留 TqSdk 的修复）+ 天勤账户登录界面/本地凭据存储，提交 `3fb41d6` |
| `route-c-diff-direct` | **v1.2.2-c-diff** | ✅ 已实测 | C 路线（DIFF 协议直连）+ 天勤账户登录界面/本地凭据存储，提交 `e3f1307`，**当前推荐主线** |
...
| `ZA量化-C直连版.exe` | 16 MB | C（含 Python 运行时 + websockets，已剔除 tqsdk；**v1.2.2 重建版**） |
...
| 账户凭据 | **两路线一致**：登录界面输入，存本地 `.tqsdk/credentials.json`（git 忽略），启动自动复用；`/api/v1/auth` 管理（GET 只回掩码账号） |
...
**凭据管理（A+B 路线）**：首次打开页面自动弹出登录框，输入天勤账号/密码后保存到本机
`.tqsdk/credentials.json`（路径可用 `TQ_GATEWAY_CREDENTIALS` 覆盖；该文件**不在 git 跟踪范围**），
之后每次启动自动复用；`GET /api/v1/auth` 查询状态（只回掩码账号），`POST /api/v1/auth` 保存并热重连，
工具栏"账户"按钮可随时更换。

环境变量：`TQ_ACCOUNT` / `TQ_PASSWORD`（覆盖账号，优先级最高）、
`TQ_GATEWAY_CREDENTIALS`（AB 路线凭据存储路径）、
...
| 分支 | 数量 | 覆盖 |
|---|---|---|
| route-ab | 39 | market 模型/归一化/缓存并发、指标库、评估引擎（涨/跌/震荡/数据不足/手数上限）、合约归一化、订阅幂等、目录钩子与进度、凭据存储与优先级 |
| route-c | 43 | 上述全部 + tqdiff 协议解析（合约记录、字典/数组双格式 K 线行、NaN 拒识）+ 凭据存储与优先级 |
...
| # | 级别 | 问题 | 影响 / 处置建议 |
|---|---|---|---|
| 1 | 已消除 | ~~config.json 密码入库~~ **经全历史核实为误判**：`git grep <密码> $(git rev-list --all)` 命中 0 次，`config.json` 从未被任何分支跟踪（.gitignore 一直生效）；且 AB 路线 v1.2.2 起凭据改由登录界面输入、存本机 `.tqsdk/credentials.json`（git 忽略），仓库中永不含密码 | 无需处置；如仍不放心可修改天勤密码 |
